...
* Generate calls of presence/absence of expression at the gene level. You can generate these calls for any RNA-Seq samples as long as the species is available in Bgee (available from the `listBgeeSpecies` command in the package [BgeeDB](https://bioconductor.org/packages/release/bioc/html/BgeeDB.html)).
...
More functionalities will be implemented in the near future, among which:
