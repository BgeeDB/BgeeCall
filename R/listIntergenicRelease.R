...
#'
#' @description Returns information on available Bgee intergenic releases, the access URL for FTP, and the date of release
#'
#' @param release A character specifying a targeted release number (e.g., "14"). If not specified, all available releases are shown.
#'
#' @return A data frame with information on Bgee intergenic releases available to use with the BgeeCall package.
#'
#' @examples{
...
# Function displaying the user a data frame describing all intergenic releases available for the BgeeCall package
list_intergenic_release <- function(release=NULL){
  ...
  return(allReleases[, c("release","releaseDate", "FTPURL", "referenceIntergenicFastaURL", "minimumVersionBgeeCall", "description", "messageToUsers")])
}

# Function returning a data frame describing all intergenic releases available for the BgeeCall package
.getIntergenicRelease <- function(removeFile=TRUE){
  ...
  if (success == 0 & file.exists(file.path(getwd(), 'release.tsv.tmp'))) {
    ...
    stop("ERROR: File describing intergenic releases could not be downloaded from FTP.")
  }
