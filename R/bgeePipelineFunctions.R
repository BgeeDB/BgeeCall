...
#' @noRd
#'
calculate_abundance_cutoff <- function(counts, selected_coding, 
                                       selected_intergenic, intergenic_cutoff = 0.05){
    ## r = (number of intergenic regions with TPM values higher than x * number 
    # of coding regions) / (number of coding regions with TPM values higher 
    # than x * number of intergenic regions) = 0.05
    ## What is value of x (cutoff)? calculate the distribution of r for a range of 
    # TPMs, then select the closest value to `cutoff`
    
    ## Counting how many intergenic regions have equal or higher value of TPM for 
    # every value of TPM
    ## For each gene's TPM (sorted), calculate r
    summed_intergenic <- vapply(unique(sort(counts$abundance[selected_coding])), 
                                function(x){
    ...
    ## Select the minimal value of TPM for which the ratio of genes and intergenic 
    # regions is equal to `intergenic_cutoff` or lower (first test if at least 1 TPM 
    # value has this property):
...
the first value with maximum coding/intergenic ratio. r=", r_cutoff,
                   "at TPM=", TPM_cutoff,"\n"))
    } else {
        TPM_cutoff <- sort(unique(
...
#'
plot_distributions <- function(counts, selected_coding, selected_intergenic, 
                               cutoff, myUserMetadata){
    ## Plotting of the distribution of TPMs for coding and intergenic 
    ## regions + cutoff
    ...
    dens_intergenic$y <- dens_intergenic$y * sum(selected_intergenic) / 
        length(counts$abundance)
...
# was used to define the reference intergenic regions.
# We do not need it anymore in this package as we precomputed the list of 
...
#' @noRd
#'
cutoff_info <- function(counts, column, abundance_cutoff, r_cutoff, 
                        myUserMetadata){
    ...
    number_genic_present <- 
        sum(counts[[column]][counts$type == "genic"] == "present")
    ...
    to_export <- c(basename(myUserMetadata@rnaseq_lib_path),
                   abundance_cutoff,
                   genic_present, number_genic_present, 
                   sum(counts$type == "genic"),
                   coding_present,  number_coding_present, 
                   sum(counts$biotype %in% "protein_coding"),
                   intergenic_present, number_intergenic_present, 
                   sum(counts$type == "intergenic"),
                   r_cutoff
    )
    names(to_export) <- c("libraryId",
                          "cutoffTPM",
                          "proportionGenicPresent", "numberGenicPresent", 
                          "numberGenic", "proportionCodingPresent", 
                          "numberPresentCoding", "numberCoding",
