...
generate_theoretical_pValue <- function(counts, pValueCutoff, method="Normal", pvalueCorrection="None") {
    ...
    if(method == "Normal"){
        ...
        tobitParameters = tobit(log2(selected_intergenic$abundance)~1, left=log2(leftCensoredThreshold), dist="gaussian")
        selected_count$zScore = (log2(selected_count$abundance) - tobitParameters$coefficients) / tobitParameters$scale
        selected_count$pValue = pnorm(log2(selected_count$abundance), mean=tobitParameters$coefficients, sd=tobitParameters$scale, lower.tail=FALSE)
    } else {
