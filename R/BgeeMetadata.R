...
#' 
#' @description An S4 class that contains all information to retrieve intergenic 
#' regions generated by Bgee.
#' 
#' @slot intergenic_release Bgee intergenic release that will be used
#' @slot intergenic_prefix String used to generate an intergenic release specific 
#' output directory
#' @slot fasta_intergenic_url URL to fasta file containing reference intergenic 
#' regions as defined by the Bgee pipeline 
#' (https://github.com/BgeeDB/bgee_pipeline/tree/master/pipeline/RNA_Seq)
#' @slot fasta_intergenic_name Name of the fasta file containing all intergenic 
#' regions that will be download to get_species_path()
...
setMethod(f = "initialize", signature ="BgeeMetadata" , 
          definition = function(.Object, ...) {
              ...
              if ( is(allReleases, "data.frame") ){
                  ...
                  if (file.exists(file.path(getwd(), "release.tsv"))){
                      cat(paste0("\nWARNING: BgeeCall could not access intergenic releases 
                 information from the internet, but a release information file was 
                 found in the download directory ",
                                 getwd(), ". This release file will be used, but be warned that it
                 may not be up to date!\n"))
                      allReleases <- read.table(file.path(getwd(), "release.tsv"), 
                                                header=TRUE, sep="\t")
                  } else {
                      stop("ERROR: BgeeCall could not access Bgee intergenic releases information. 
           Is your internet connection working?")
              ...
              if (length(.Object@intergenic_release) == 0) {
                  ...
                  .Object@fasta_intergenic_url <- 
                      as.character(allReleases$referenceIntergenicFastaURL[1])
                  ...
                  if (.Object@intergenic_release %in% allReleases$release) {
                      .Object@intergenic_release <- 
                          as.character(allReleases$release[allReleases$release == 
                                                               .Object@intergenic_release])
                      .Object@fasta_intergenic_url <- 
                          as.character(allReleases$referenceIntergenicFastaURL[allReleases$release == 
                                                                                   .Object@intergenic_release])
                      message_to_users <- 
                          as.character(allReleases$messageToUsers[allReleases$release == 
                                                                      .Object@intergenic_release])
                  } else {
                      stop("ERROR: The specified release number is invalid, or is not available 
             for BgeeCall.")
                  }
              } else {
