...
#' }
#' 
generate_calls_workflow <- function (myAbundanceMetadata = new("KallistoMetadata"), 
                                     myBgeeMetadata = new("BgeeMetadata"), 
                                     userMetadata = NULL, userDataFrame = NULL,
                                     userFile = NULL) {
    if (is.null(userMetadata) && is.null(userDataFrame) && 
        ...
        if (typeof(userMetadata) == "S4") {
            ...
            stop("the parameter userMetadata should only be used to provide one or a list of UserMetadata objects")
        }
        ...
        # run workflow when userDataFrame is not null   
               ...
               !is.null(userFile)) {
        if ( (!(typeof(userFile) == "character") && file.exists(userFile)) ) {
            stop("Please provide a path to the file that contains all information
                 allowing to generate UserMetadata objects")
        }
