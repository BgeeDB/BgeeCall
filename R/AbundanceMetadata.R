...
#' @slot ignoreTxVersion logical used to remove transcript version in transcript ID
#' if TRUE (default = FALSE)
#' @slot cutoff numeric corresponding to the proportion of intergenic present 
#' divided by proportion of protein coding present (default = 0.05). In the Bgee 
#' pipeline this cutoff is fixed and its value is 0.05. Be careful when changing 
#' this parameter as it could have a huge impact on your present/absent calls.
...
#' You should edit this slot only if you already have such a file with a different
#' name.
#' @slot tx2gene_file Name of the file containing the mapping between transcript 
#' IDs and gene IDs (See the tximport package vignette for more details). 
#' This file is created by the pipeline. You should edit this slot only if you 
#' already have such a file with a different name. This file must be store at 
#' get_species_path()
...
#' @slot gene2biotype_file Name of the file containing the mapping between gene IDs
#' and biotypes. This file is created by the pipeline. You should edit this slot 
#' only if you already have such a file with a different name.
...
#' @slot abundance_file Name of the transcript-level abundance file. All descendant
#' of this class have to define a value for this slot (in the prototype section)
#' @slot read_size_kmer_threshold read size of the library below which transcript 
#' index is created using a smaller kmer size
...
#' @slot eff_length_header Name of the header of the column that contains effective
#' length
#' @slot transcript_calls_file_name default name of file containing all transcript 
#' ids and calls (if calls created at transcript level)
#' @slot gene_calls_file_name default name of file containing all gene ids and 
#' calls
#' (if calls created at gene level)
#' @slot transcript_cutoff_file_name default name of file containing summary of 
#' cutoff used to generate transcript expression calls (if calls created at 
#' transcript level)
#' @slot gene_cutoff_file_name default name of file containing summary of cutoff 
#' used to generate gene expression calls (if calls created at gene level)
#' @slot transcript_distribution_file_name default name of density plot file 
#' containing TPM distribution of all transcripts  (if calls created at transcript 
#' level)
#' @slot gene_distribution_file_name default name of density plot file containing 
#' TPM distribution of all genes (if calls created at gene level)
#' 
#' 
AbundanceMetadata <- setClass(
    ...
    prototype = prototype(
        ...
        tx2gene_file_without_version = "tx2gene_without_version.tsv",
