...
#' # load transcriptome from AnnotationHub
...
#' user <- setTranscriptomeFromObject(user, transcriptome_object,
#'                                 "WBcel235")
#' merge_transcriptome_and_intergenic(kallisto, bgee, user)
#' }

merge_transcriptome_and_intergenic <- function (myKallistoMetadata, myBgeeMetadata,
                                                myUserMetadata) {
    ...
    transcriptome_with_intergenic_path <- 
        file.path(transcriptome_dir, myKallistoMetadata@full_transcriptome_file)
    ...
    if (file.exists(transcriptome_with_intergenic_path)) {
        cat("File containing both transcriptomic and intergenic regions already 
        exists.\n")
    } else {
        cat("Start generation of the file containing both transcriptomic and 
        intergenic regions.\n")
        ...
        cat("File containing both transcriptomic and intergenic regions has been 
