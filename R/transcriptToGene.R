...
create_tx2gene <- function(myAbundanceMetadata, myBgeeMetadata, myUserMetadata) {
  ...
  tx2gene_path <- file.path(annotation_path, myAbundanceMetadata@tx2gene_file)
  if (!file.exists(tx2gene_path)) {
    cat(paste0("Generate file ", myAbundanceMetadata@tx2gene_file, ".\n"))
...
run_tximport <- function (myAbundanceMetadata, myBgeeMetadata, myUserMetadata) {
  tx2gene_path <- create_tx2gene(myAbundanceMetadata, myBgeeMetadata, myUserMetadata)
...
#' @noRd
#'
abundance_without_intergenic <- function (myAbundanceMetadata, myBgeeMetadata, myUserMetadata) {
  file_without_intergenic_name <- "abundance_without_intergenic.tsv"

  # remove intergenic from tx2gene
  tx2gene_path <- create_tx2gene(myAbundanceMetadata, myBgeeMetadata, myUserMetadata)
  tx2gene <- read.table(tx2gene_path,header = TRUE, sep = "\t")
  tx2gene_without_intergenic <- tx2gene[as.character(tx2gene$TXNAME)!=as.character(tx2gene$GENEID),]
